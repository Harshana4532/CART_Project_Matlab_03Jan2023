function [fig3, xmn, Rmn1, Hmn1]=Plot_synN_HD_ODE45_SYNCONS_CONS_FinSH(k, Kph, N_hd, A, mu, sig, hil,np)

global cutoff_time cal calR2 minS binS maxS 

        figure(19803);clf; hold on
        figure(198003);clf; hold on
        figure(298003);clf; hold on
        warning('off')
...
xlabel('time (days)')
title('Fig.4D/S3: synN High, log10(HER-2)=6.3')
...
xlabel('Time (days)')
title('Fig.4E/S3: synN High, log10(HER-2)=6.3')
...
ylabel('\rho_{H,R}.mean(U_H)')
xlabel('Time (days)')
title('Fig. S3: synN High, log10(HER-2)=4.7')
...
figure(20);  hold on
fig3=plot((data_HD(:,1)),YY,'k-', 'LineWidth',2);
plot((data_HD(:,1)),data_HD(:,2),'diamond','MarkerSize',10, 'Color','k' );
