...
function [fig1, xmn, Rmn1, Hmn1]=Plot_synN_HD_ODE45_SYNCONS_CONS_FinCH2(k, A, mu, sig)
...
%%%%%%%%%%%%%%% Binned LN T data w.r.t. R at time zero
...
%%%%%%%%%%%%%%%% LN U data w.r.t. H at time zero
dataUH1=[24	20000];
...
Hmn1(p)=sum(H.*Uh(1,:)'./sum(Uh(1,:)));
    xmn(p)=0.5.*(Rmn1(p)+Hmn1(p)+KD).*(1-sqrt(1-(4.*Rmn1(p).*Hmn1(p))./((Rmn1(p)+Hmn1(p)+KD).^2)));
Lam_xmn(p)=k.*xmn(p);
rho_xmn(p)=A.*xmn(p);
...
%sum_T_Haff(p)=sum(yR(end,lenH+1:lenR));
%%%%%%%
