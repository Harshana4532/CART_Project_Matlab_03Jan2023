function [fig1, xmn, Rmn1, Hmn1]=Plot_synN_HD_ODE45_SYNCONS_CONS_FinCH2(k, Kph, N_hd, A, mu, sig)

global cutoff_time cal calR2 minS binS maxS

        figure(19801); clf; hold on
        figure(29801); clf; hold on
...
xlabel('Time (days)')
title('Fig.3A/S2: Const High, log10(HER-2)=6.3')
KILL3x=(mean((Lam(:,5).*yR(:,lenH+1:end)')))';
hold on
plt3=plot(tR, KILL3x, '-.','LineWidth', 4,'Color','k');
legend([plt1(1) plt3(1)],{'\lambda_{R,H}.T_R(t)','mean(\lambda_{R,H}.T_R(t))'},'Location','northwest')
axis([0,3,0,0.25])
box on
end
...
legend boxoff
legend off
ylabel('\rho_{H,R}.U_H(t)')
xlabel('Time (days)')
title('Fig.3B/S2: Const High, log10(HER-2)=6.3')
KILL4x=(mean((rho(:,5).*mean(yR(:,1:lenH)'))))';
hold on
plt3=plot(tR, KILL4x, ':','LineWidth', 2,'Color','k');
legend([plt1(1) plt3(1)],{'\rho_{R,H}.U_H(t)','mean(\rho_{R,H}.U_H(t))'},'Location','northwest')
axis([0,3,0,0.4])
box on
end
...
ylabel('\lambda_{H,R}.T_R(t)')
xlabel('Time (days)')
title('Fig.S2: Const High, log10(HER-2)=4.7')
KILL3x=(mean((Lam(:,5).*yR(:,lenH+1:end)')))';
hold on
plt3=plot(tR, KILL3x, '-.','LineWidth', 4,'Color','k');
legend([plt1(1) plt3(1)],{'\lambda_{R,H}.T_R(t)','mean(\lambda_{R,H}.T_R(t))'},'Location','northwest')
