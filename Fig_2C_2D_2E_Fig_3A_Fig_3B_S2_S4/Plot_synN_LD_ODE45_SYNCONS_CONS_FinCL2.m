function [fig2, xmn, Rmn1, Hmn1]=Plot_synN_LD_ODE45_SYNCONS_CONS_FinCL2(k, Kph, N_hd, A, mu, sig)       

global cutoff_time cal calR2 minS binS maxS

        figure(19802);clf; hold on
        figure(29802);clf; hold on
...
xlabel('Time (days)')
title('Fig.S2: Const Low, log10(HER-2)=6.3')
...
ylabel('\lambda_{H,R}.T_R(t)')
xlabel('Time (days)')
title('Fig.S2: Const Low, log10(HER-2)=4.7')
KILL3x=(mean((Lam(:,5).*yR(:,lenH+1:end)')))';
hold on
plt3=plot(tR, KILL3x, '-.','LineWidth', 4,'Color','k');
legend([plt1(1) plt3(1)],{'\lambda_{R,H}.T_R(t)','mean(\lambda_{R,H}.T_R(t))'},'Location','northwest')
